...
void			displayGui(){
	...
	ImGui::Begin("Diffraction-Control");
	ImGui::SetWindowSize(ImVec2(200, 200), ImGuiSetCond_Once);
	ImGui::SetWindowPos(ImVec2(220, 10), ImGuiSetCond_Once);

	if (ImGui::TreeNode("Light Position"))
	{
		ImGui::SliderFloat3("Position", glm::value_ptr(light_position), -10.0f, 10.0f, "%.2f");
		ImGui::Separator();
		ImGui::InputFloat("X", &light_position[0], 0.1, 0.2, 2);
		ImGui::InputFloat("Y", &light_position[1], 0.1, 0.2, 2);
		ImGui::InputFloat("Z", &light_position[2], 0.1, 0.2, 2);
		ImGui::TreePop();
	}
	ImGui::Separator();
	ImGui::SliderFloat("Grating Spacing", &GratingSpacing, 0.0, 10.0, "%.2f");
	ImGui::Separator();
	ImGui::InputFloat("Surface Roughness", &SurfaceRoughness, 0.0, 10.0, 2);
	ImGui::Separator();
	ImGui::ColorEdit3("Highlight Color", (float*)&HighlightColor);
	ImGui::End();
}
...
void init3Dmodel()
{
	...
	GLuint vTangentMesh = glGetAttribLocation(program3Dmodel, "MCTangent");
