...
Sphere lightSourceVolume = Sphere(20.0, vec3(50.0, 81.60, 81.60), vec3(12.0), vec3(0.0), DIFF);
...
void initSpheres() {
	...
	spheres[8] = Sphere(600.0, vec3(50., 681.33, 81.6),	vec3(12.), vec3(0.), DIFF);
}
...
void main() {
	...
	//vec3 camPos = vec3((2.0 * (mouse.xy==vec2(0.0)?0.5*resolution.xy:mouse.xy) / resolution.xy - 1.0) * vec2(48.0, 40.0) + vec2(50.0, 40.8), 169.0);
	vec3 camPos = vec3(30.0,40.0,170.0);
	...
	for (int i = 0; i < SAMPLES; ++i){
		#ifdef DEBUG
        	vec3 test = radiance(Ray(camPos, normalize(.53135 * (resolution.x/resolution.y*uv.x * cx + uv.y * cy) + cz)));
	...
	gl_FragColor = vec4(pow(clamp(color/float(SAMPLES), 0.0, 1.0), vec3(1./2.2)), 1.0);
}