...
/**
	Marios Kanakis - 28.08.16
	Modified minimalOpenVR.h

	Changed getEyeTransformations function to take and "return" 
	column-major matrices instead of row-major one's used by 
	Morgan McGuire's code.

	The change is small and thus, the header file is still
	credited to Morgan, leaving the file name untouched.

	Description at: main.cpp
*/
...
void getEyeTransformations
	...
	float				nearPlaneZ,
	float				farPlaneZ,
	float*				headToWorldColumnMajor3x4,
	float*				ltEyeToHeadColumnMajor3x4,
	float*				rtEyeToHeadColumnMajor3x4,
	float*				ltProjectionMatrixColumnMajor4x4,
	float*				rtProjectionMatrixColumnMajor4x4)
{
...
#   endif
	...
	for (int r = 0; r < 3; ++r) {
		for (int c = 0; c < 4; ++c) {
			ltEyeToHeadColumnMajor3x4[r + c * 4] = ltMatrix.m[r][c];
			rtEyeToHeadColumnMajor3x4[r + c * 4] = rtMatrix.m[r][c];
			headToWorldColumnMajor3x4[r + c * 4] = head.m[r][c];
	...
	for (int r = 0; r < 4; ++r) {
		for (int c = 0; c < 4; ++c) {
			ltProjectionMatrixColumnMajor4x4[r + c * 4] = ltProj.m[r][c];
			rtProjectionMatrixColumnMajor4x4[r + c * 4] = rtProj.m[r][c];
		}
	}
